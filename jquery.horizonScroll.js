...
var tmpi = getHashedPosition();
    ...
    var sizeSections = function () {
        ...
        $.fn.horizon.defaults.i = tmpi;
        setAsActive($.fn.horizon.defaults.i, true);
    ...
    function setAsActive(index, override) {
        if(override) {
                scrollTo(index,0);
        }
        rewriteHash(index);
        if ($("#slide-accessor-" + (index+1)).length > 0) {
            $('#slide-accessors').children('li').each(function () {
              $(this).removeClass("active");
            });
            $("#slide-accessor-" + (index+1)).addClass("active");
        } else {
            console.log('out of range');
        }
    }
    function rewriteHash(index)  {
        setTimeout(function(){
            window.location.hash = ("section-section"+(index+1));
        },600);
